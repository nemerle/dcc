...
static int format_seg( x86_op_t *op, char *buf, int len,
        ...
        switch (op->flags.op_seg) {
                ...
                case x86_op_flags::op_ds_seg: reg = "ds"; break;
                case x86_op_flags::op_fs_seg: reg = "fs"; break;
                case x86_op_flags::op_gs_seg: reg = "gs"; break;
                default:
                        break;
        }
...
int x86_op_t::x86_format_operand(char *buf, int len,
        ...
        switch ( format ) {
                case att_syntax:
                        return format_operand_att( this, _insn, buf, len );
                case xml_syntax:
                        return format_operand_xml( this, _insn, buf, len );
                case raw_syntax:
                        return format_operand_raw( this, _insn, buf, len );
                case native_syntax:
                case intel_syntax:
                default:
                        return format_operand_native( this, _insn, buf, len );
        }
}
char * x86_op_t::format( enum x86_asm_format format ) {
    char *buf, *str;
    size_t len;
    ...
    x86_format_operand( buf, len, format );
...
static int format_att_mnemonic( x86_insn_t *insn, char *buf, int len) {
        ...
        if (!(insn->note & insn_note_nosuffix) &&
            (insn->group == x86_insn_t::insn_arithmetic ||
             insn->group == x86_insn_t::insn_logic ||
             insn->group == x86_insn_t::insn_move ||
             insn->group == x86_insn_t::insn_stack ||
             insn->group == x86_insn_t::insn_string ||
             insn->group == x86_insn_t::insn_comparison ||
             insn->type == insn_in ||
             insn->type == insn_out
            )) {
...
static int format_raw_insn( x86_insn_t *insn, char *buf, int len ){
    ...
    opstr.len = len;
    insn->x86_operand_foreach( format_op_raw, &opstr, op_any );
...
int X86_Disasm::x86_format_header( char *buf, int len, enum x86_asm_format format ) {
        switch (format) {
                case att_syntax:
...
int x86_insn_t::x86_format_insn( char *buf, int len,
    ...
    if ( format == intel_syntax ) {
        ...
        STRNCAT( buf, prefix_string, len );
        STRNCAT( buf, mnemonic, len );
        STRNCAT( buf, "\t", len );
        ...
        if ( (src = x86_operand_2nd()) ) {
            ...
            src->x86_format_operand( str, MAX_OP_STRING, format);
            STRNCAT( buf, str, len );
        }
        ...
        STRNCAT( buf, prefix_string, len );
        format_att_mnemonic(this, str, MAX_OP_STRING);
        STRNCATF( buf, "%s\t", str, len);
        ...
        * an imm as the first operand, sometimes as the third... */
        ...
        format_raw_insn( this, buf, len );
    } else if ( format == xml_syntax ) {
        format_xml_insn( this, buf, len );
    } else { /* default to native */
        /* NATIVE style: RVA\tBYTES\tMNEMONIC\tOPERANDS */
        /* print address */
        STRNCATF( buf, "%08" PRIX32 "\t", addr, len );

        /* print bytes */
        for ( i = 0; i < size; i++ ) {
            STRNCATF( buf, "%02X ", bytes[i], len );
        }
