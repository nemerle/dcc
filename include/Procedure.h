...
protected:
    ...
    Function(FunctionType*/*ty*/) : hasCase(false)
    {
...
public:
    FunctionType* type;
    CConv* m_call_conv;
    uint32_t procEntry = 0; /* label number                         	 */
    QString name; /* Meaningful name for this proc     	 */
    STATE state; /* Entry state                          	 */
    int depth = 0; /* Depth at which we found it - for printing */
    uint32_t flg = 0; /* Combination of Icode & Proc flags    	 */
    int16_t cbParam = 0; /* Probable no. of bytes of parameters  	 */
    ...
    size_t numBBs = 0; /* Number of BBs in the graph cfg       	 */
    ...
    bool liveAnal = false; /* Procedure has been analysed already		 */
